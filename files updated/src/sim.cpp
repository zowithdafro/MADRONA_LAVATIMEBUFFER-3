...
/*
void Sim::updateLavaInteraction() {
    for (CountT i = 0; i < consts::numAgents; i++) {
        Entity agent = agents[i];
        Position agentPos = get<Position>(agent);

        // Iterate through all lava entities
        for (const auto& lavaEntity : lavaEntities) { // Assuming lavaEntities is a container of lava Entity IDs
            if (lavaEntity == Entity::none()) continue;
            Position lavaPos = get<Position>(lavaEntity);
            float distance = (agentPos - lavaPos).length();
            // Check for collision based on distance
            if (distance < consts::lavaCollisionRadius) {
                handleAgentLavaCollision(agent);
                // Break out of the loop if a collision is found
                break;
            }
        }
    }
...
void Sim::handleAgentLavaCollision(Entity agent) {
    AgentState& agentState = get<AgentState>(agent);
    agentState.isDead = true;
    std::cout << "Agent " << agent << " is dead due to lava collision." << std::endl;
...
void Sim::setupTasks(TaskGraphBuilder &builder, const Config &cfg)
{
    ...
    auto lava_sys = builder.addToGraph<ParallelForNode<Engine,
        ...
        >>({phys_done});
    /*
    auto update_lava_task = builder.addToGraph<ParallelForNode<Engine,
                &Sim::updateLavaInteraction,
                Position,
            >>({phys_done, agent_zero_vel});
*/
